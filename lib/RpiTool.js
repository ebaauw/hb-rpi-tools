...
Paramerers:
  ...
  The sensor is read from ${b(GpioClient.ds18b20.prefix)}${u('id')}${b(GpioClient.ds18b20.postfix)}.`,
  ...
  async ds18b20 (...args) {
    ...
    parser
      ...
      .parameter('id', (value) => {
        ...
        if (!GpioClient.ds18b20.validId(clargs.id)) {
          throw new UsageError(`${clargs.id}: invalid DS18B20 ID`)
        }
      })
      .parse(...args)
    const temp = await this.pi.readFile(GpioClient.ds18b20.fileName(clargs.id))
