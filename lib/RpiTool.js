// hb-rpi-tools/lib/RpiTool.js
// Copyright © 2019-2026 Erik Baauw.  All rights reserved.
//
// Homebridge RPi Tools.

import { toHexString, timeout } from 'hb-lib-tools'
...
const ds18b20 = GpioClient.ds18b20
...
const description = {
  ...
  ds18b20: 'Read DS18B20 temperature sensor.',
...
Paramerers:
  ...
  ds18b20: `${description.ds18b20}

Usage: ${b('rpi')} ${usage.ds18b20}
...
Paramerers:
  ...
  ${u('id')}
  ID of the DS18B20 sensor.
  The sensor is read from ${b(ds18b20.prefix)}${u('id')}${b(ds18b20.postfix)}.`,
...
class RpiTool extends CommandLineTool {
  constructor (pkgJson) {
  ...
  async ds18b20 (...args) {
    ...
    parser
      .help('h', 'help', this.help)
      .parameter('id', (value) => {
        clargs.id = OptionParser.toString('id', value, true)
        if (!ds18b20.validId(clargs.id)) {
          throw new UsageError(`${clargs.id}: invalid DS18B20 ID`)
        }
      })
      .parse(...args)
    const temp = await this.pi.readFile(ds18b20.fileName(clargs.id))
    this.print('temperature: %dº', parseInt(temp) / 1000)
  }
